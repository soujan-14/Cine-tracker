...
export function getImageUrl(path: string | null | undefined, size: 'w300' | 'w500' | 'w780' | 'w1280' | 'original' = 'w500'): string {
...
export async function getMovieDetails(id: number | string): Promise<MovieDetails> { if (!id) throw new Error('Movie ID is required'); return fetchFromTmdb(`/movie/${id}`); }
export async function getMovieCredits(id: number | string): Promise<MovieCredits> { if (!id) throw new Error('Movie ID is required'); return fetchFromTmdb(`/movie/${id}/credits`); }
export async function getMovieVideos(id: number | string): Promise<MovieVideosResponse> { if (!id) throw new Error('Movie ID is required'); return fetchFromTmdb(`/movie/${id}/videos`); }
export async function getSimilarMovies(id: number | string, page = 1): Promise<TmdbPaginatedResponse<Movie>> { if (!id) throw new Error('Movie ID is required'); return fetchFromTmdb(`/movie/${id}/similar`, { page }); }
export async function getRecommendedMovies(id: number | string, page = 1): Promise<TmdbPaginatedResponse<Movie>> { if (!id) throw new Error('Movie ID is required'); return fetchFromTmdb(`/movie/${id}/recommendations`, { page }); }
export async function getWatchProviders(id: number | string): Promise<WatchProvidersResult> { if (!id) throw new Error('Movie ID is required'); return fetchFromTmdb(`/movie/${id}/watch/providers`); }
