...
export function NotificationManager() {
  ...
  useEffect(() => {
    ...
    const onSearchClick = (event: Event) => { event.preventDefault(); setSearchOpen(true); };
  ...
  useEffect(() => {
    ...
    if (favoriteUpcoming) new Notification('⭐ Your favorite movie is coming soon!', { body: `${favoriteUpcoming.title} releases ${favoriteUpcoming.release_date || 'soon'}.` });
  ...
  return <>
    <AnimatePresence>{searchOpen && <motion.div className="fixed inset-0 z-[100] bg-black/90 p-4 backdrop-blur-xl sm:p-8" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}><div className="mx-auto w-full max-w-3xl pt-4 sm:pt-12"><div className="flex items-center gap-3 rounded-2xl border border-white/15 bg-[#141414] px-4 py-3 shadow-2xl"><Search className="h-5 w-5 text-white/50" /><input ref={inputRef} value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => { if (e.key === 'Escape') closeSearch(); if (e.key === 'Enter' && results[0]) openResult(results[0].id); }} placeholder="Search movies…" className="w-full bg-transparent text-lg text-white outline-none" aria-label="Movie search" />{isFetching && <Loader2 className="h-5 w-5 animate-spin text-white/40" />}<button onClick={closeSearch} aria-label="Close search" className="rounded-full p-2 text-white/60 hover:bg-white/10 hover:text-white"><X className="h-5 w-5" /></button></div><div className="mt-4 overflow-hidden rounded-2xl border border-white/10 bg-[#141414]">{query && results.length === 0 && !isFetching ? <p className="p-6 text-center text-white/50">No movies found.</p> : results.map((movie, index) => <button key={movie.id} onClick={() => openResult(movie.id)} className={`flex w-full items-center gap-4 border-b border-white/5 p-3 text-left hover:bg-white/5 ${index === 0 ? 'bg-white/[0.04]' : ''}`}><div className="relative h-16 w-11 overflow-hidden rounded-md bg-white/5">{movie.poster_path && <Image src={getImageUrl(movie.poster_path, 'w300')} alt={movie.title} fill sizes="44px" className="object-cover" />}</div><div className="min-w-0"><p className={`${index === 0 ? 'text-base font-bold' : 'text-sm font-semibold'} truncate text-white`}>{movie.title}</p><p className="text-xs text-white/45">{movie.release_date?.slice(0, 4) || '—'}</p></div></button>)}</div></div></motion.div>}</AnimatePresence>
    {typeof window !== 'undefined' && 'Notification' in window && !permissionGranted ? <button onClick={async () => setPermissionGranted((await Notification.requestPermission()) === 'granted')} className="fixed bottom-5 right-5 z-40 hidden rounded-full border border-white/10 bg-[#141414] p-3 text-white/60 shadow-xl hover:text-white sm:flex" title="Enable movie notifications" aria-label="Enable movie notifications"><Bell className="h-5 w-5" /></button> : null}
  </>;
}
