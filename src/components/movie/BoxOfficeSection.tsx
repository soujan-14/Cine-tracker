...
export function BoxOfficeSection({ data, isLoading }: Props) {
  ...
  const hasAnyData = data && data.source !== 'none' &&
    (data.worldwide || data.domestic || data.budget);

  if (!hasAnyData) {
    return (
      <div className="rounded-xl border border-white/10 bg-neutral-900/50 px-5 py-4">
        <p className="text-sm text-white/40">Box office data unavailable</p>
      </div>
    );
  }

  const stats = [
    data.worldwide && { label: '💰 Worldwide', value: fmt(data.worldwide) },
    data.domestic && { label: '🇺🇸 Domestic', value: fmt(data.domestic) },
    data.budget && { label: '🎬 Budget', value: fmt(data.budget) },
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        ...
        {data.status && (
          <span
            className={`rounded-full border px-3 py-0.5 text-xs font-bold ${STATUS_STYLES[data.status]}`}
          >
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
        {stats.map((stat) => (
          <div
            key={stat.label}
            className="rounded-xl border border-white/10 bg-neutral-900 px-4 py-4"
          >
