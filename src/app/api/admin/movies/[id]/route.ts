...
function normalizeDate(value: unknown): Date | null | 'invalid' {
  if (value === undefined || value === null || value === '') return null;
...
function normalizeUrl(value: unknown, field: string): string | null | NextResponse {
  ...
  try {
    ...
    const movie = await prisma.movie.update({
      ...
      data: {
        tmdbId,
        title,
        originalTitle: normalizeString(body.originalTitle),
        overview: normalizeString(body.overview),
        customPoster,
        customBackdrop,
        customTrailer,
        customCast: Array.isArray(body.customCast) ? body.customCast : null,
        customCrew: Array.isArray(body.customCrew) ? body.customCrew : null,
        customGenres: Array.isArray(body.customGenres) ? body.customGenres : null,
        releaseDate,
    ...
    revalidatePath(`/admin/movies/${movie.id}/edit`);
