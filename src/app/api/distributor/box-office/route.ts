  ...
  try {
    ...
    if (!movie) {
      ...
      movie = await prisma.movie.create({
        data: { id: movieId, tmdbId: movieId > 0 ? movieId : null, title },
      });
    }
    ...
    const existing = await prisma.boxOffice.findFirst({
      where: {
        movieId,
    ...
    const record = existing
      ? await prisma.boxOffice.update({
          ...
          data: { dailyCollection, weekendCollection, collectionDate, territory, verified: auth.role === 'ADMIN' ? true : existing.verified },
        })
      : await prisma.boxOffice.create({
          data: { movieId, distributorId: auth.userId, dailyCollection, weekendCollection, collectionDate, territory, verified: auth.role === 'ADMIN' },
        });
