...
function parseDate(value: unknown): Date | null { if (value === undefined || value === null || value === '') return null; const date = new Date(String(value)); return Number.isNaN(date.getTime()) ? null : date; }
...
export async function GET(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  const record = await prisma.boxOffice.findUnique({ where: { id }, include: { movie: { select: { id: true, title: true } }, distributor: { select: { id: true, name: true, email: true } } } });
  if (!record) return NextResponse.json({ error: 'Collection record not found.' }, { status: 404 });
  if (auth.role !== 'ADMIN' && record.distributorId !== auth.userId) return NextResponse.json({ error: 'Forbidden.' }, { status: 403 });
...
export async function PUT(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (auth.role !== 'ADMIN' && existing.distributorId !== auth.userId) return NextResponse.json({ error: 'Forbidden.' }, { status: 403 });
    ...
    const record = await prisma.boxOffice.update({
      ...
      include: { movie: { select: { id: true, title: true } }, distributor: { select: { id: true, name: true, email: true } } },
    });

    revalidatePath('/box-office'); revalidatePath('/admin'); revalidatePath('/distributor'); revalidatePath(`/movie/${existing.movieId}`);
    return NextResponse.json(record, { headers: { 'Cache-Control': 'no-store' } });
  } catch (error) {
