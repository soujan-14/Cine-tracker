...
export default function AdminBoxOfficeEditPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        setSubmittedBy(data.distributor?.name ?? data.distributor?.email ?? 'Unknown distributor');
  ...
  return <main className="min-h-screen bg-[#0B0B0B] px-4 py-6 text-white sm:px-8 lg:px-16"><div className="mx-auto max-w-4xl"><div className="flex flex-wrap items-center justify-between gap-4 border-b border-white/10 pb-6"><div className="flex min-w-0 items-center gap-3 sm:gap-4"><Logo size={42} textSize="sm" /><div className="min-w-0"><p className="text-xs font-semibold uppercase tracking-[0.2em] text-[#E50914]">CINE TRACKER</p><h1 className="mt-1 text-2xl font-black sm:text-4xl">Edit Collection</h1><p className="mt-1 text-sm text-white/50">{movieTitle || 'Box-office record'} · Submitted by {submittedBy || '—'}</p></div></div><Link href="/admin" className="inline-flex min-h-11 items-center gap-2 rounded-xl border border-white/10 bg-white/5 px-4 py-2 font-semibold text-white/80"><ArrowLeft className="h-4 w-4" /> Back to Dashboard</Link></div>{loading ? <p className="py-12 text-center text-white/50">Loading collection…</p> : <form onSubmit={submit} className="mt-8 grid gap-5 rounded-2xl border border-white/10 bg-white/[0.03] p-5 sm:p-6 md:grid-cols-2"><label className="md:col-span-2"><span className="mb-2 block text-sm text-white/70">Movie</span><input value={movieTitle} disabled className="min-h-11 w-full rounded-xl border border-white/10 bg-black/40 px-4 py-3 text-white/60" /></label><label><span className="mb-2 block text-sm text-white/70">Collection date</span><input required type="date" value={form.collectionDate} onChange={(e)=>set('collectionDate',e.target.value)} className="min-h-11 w-full rounded-xl border border-white/10 bg-black/30 px-4 py-3" /></label><label><span className="mb-2 block text-sm text-white/70">Territory / region</span><input value={form.territory} onChange={(e)=>set('territory',e.target.value)} className="min-h-11 w-full rounded-xl border border-white/10 bg-black/30 px-4 py-3" /></label><label><span className="mb-2 block text-sm text-white/70">Daily collection</span><input min="0" step="0.01" type="number" value={form.daily} onChange={(e)=>set('daily',e.target.value)} className="min-h-11 w-full rounded-xl border border-white/10 bg-black/30 px-4 py-3" /></label><label><span className="mb-2 block text-sm text-white/70">Weekend collection</span><input min="0" step="0.01" type="number" value={form.weekend} onChange={(e)=>set('weekend',e.target.value)} className="min-h-11 w-full rounded-xl border border-white/10 bg-black/30 px-4 py-3" /></label><div className="flex flex-wrap items-center gap-3 md:col-span-2"><button disabled={saving} className="inline-flex min-h-11 items-center gap-2 rounded-xl bg-[#E50914] px-6 py-3 font-bold disabled:opacity-60"><Save className="h-4 w-4" />{saving?'Saving…':'Save Changes'}</button><Link href="/admin" className="inline-flex min-h-11 items-center rounded-xl border border-white/10 px-6 py-3 font-semibold">Cancel</Link><span role="status" className="text-sm text-white/50">{message}</span></div></form>}</div></main>;
}