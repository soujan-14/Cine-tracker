    ...
    const load = async () => {
      ...
      try {
        const response = await fetch(
          `/api/distributor/box-office/${encodeURIComponent(params.id)}`,
          {
            headers: {
        ...
        setMovieTitle(
          data.movie?.title ?? `Movie ${data.movieId}`,
        );
        ...
        setForm({
          collectionDate: new Date(data.collectionDate)
            ...
            .slice(0, 10),
          daily:
            data.dailyCollection != null
              ? String(data.dailyCollection)
              : '',
          weekend:
            data.weekendCollection != null
              ? String(data.weekendCollection)
              : '',
      ...
      const response = await fetch(
        `/api/distributor/box-office/${encodeURIComponent(params.id)}`,
        {
          ...
          body: JSON.stringify({
            ...
            dailyCollection: form.daily || null,
            weekendCollection: form.weekend || null,
          ...
          <div className="flex min-w-0 items-center gap-3 sm:gap-4">
            ...
            <div className="min-w-0">
              ...
              <p className="mt-1 text-sm text-white/50">
                {movieTitle || 'Box-office record'} · Submitted by{' '}
                {submittedBy || '—'}
              </p>
            </div>
          </div>
          ...
          >
            <label className="md:col-span-2">
              ...
              <input
                value={movieTitle}
                disabled
                className="min-h-11 w-full rounded-xl border border-white/10 bg-black/40 px-4 py-3 text-white/60"
              />
            </label>
            ...
            <label>
              ...
              <input
                ...
                onChange={(event) =>
                  set('collectionDate', event.target.value)
                }
            ...
            <label>
              ...
              <input
                ...
                onChange={(event) =>
                  set('territory', event.target.value)
                }
                className="min-h-11 w-full rounded-xl border border-white/10 bg-black/30 px-4 py-3 text-white outline-none focus:border-[#E50914]/60"
              />
            </label>
            ...
            <label>
              ...
              <input
                ...
                value={form.weekend}
                onChange={(event) =>
                  set('weekend', event.target.value)
                }
                className="min-h-11 w-full rounded-xl border border-white/10 bg-black/30 px-4 py-3 text-white outline-none focus:border-[#E50914]/60"
              />
            </label>
